"""LangGraph orchestration for the JASPER validation pipeline."""
...
from hypothesis_agent.llm import BaseLLM, LLMError, NvidiaLLM
...
class LangGraphValidationOrchestrator:
        ...
        *,
        ...
        if llm is not None:
            ...
            if not self.settings.nvidia_api_key:
                raise LLMError("NVIDIA API key must be configured for validation pipeline")
            self.llm = NvidiaLLM(api_key=self.settings.nvidia_api_key, model=self.settings.nvidia_model)
    ...
    def _run_delivery(self, request: HypothesisRequest, context: StageContext) -> StageExecutionResult:
        ...
        context.setdefault("insights", []).append("Report ready for download via the JASPER portal.")
        self._append_workflow_log(
            context,
    ...
    def _render_report_pdf(
        ...
        *,
        ...
        # --- Header ---
        story.append(Paragraph("JASPER", styles["ReportTitle"]))
        ...
        info_data = [
            [Paragraph("<b>Hypothesis:</b>", styles["JasperBody"]), Paragraph(hypothesis_text, styles["JasperBody"])],
            [Paragraph("<b>Workflow ID:</b>", styles["JasperBody"]), Paragraph(workflow_id, styles["JasperBody"])],
            [Paragraph("<b>Entities:</b>", styles["JasperBody"]), Paragraph(", ".join(request.entities) or "N/A", styles["JasperBody"])],
        ]
        ...
        if exec_summary:
            ...
            story.append(Paragraph(str(exec_summary), styles["JasperBody"]))
        ...
        if narrative:
            ...
            story.append(Paragraph(narrative, styles["JasperBody"]))
        ...
        story.append(Paragraph(
            f"<i>Generated by JASPER • {datetime.now().strftime('%Y-%m-%d %H:%M')}</i>",
